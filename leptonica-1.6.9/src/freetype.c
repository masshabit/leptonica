	...
	for (row = 0; row < bitmap.rows; row++) {
		...
		for (col = 0; col < bitmap.width; col++, pc++) {
			...
			if (bitmap.pixel_mode == ft_pixel_mode_grays) {
				srcAlpha = (bitmap.buffer[pc] * 255 / (bitmap.num_grays - 1));
			} else if (bitmap.pixel_mode == ft_pixel_mode_mono) {
				srcAlpha = ((bitmap.buffer[(col>>3)+pcr]) & (1<<(~col&0x07))) ? 255 : 0;
			} else {
				return (PIX *)ERROR_PTR("unsupported ft_pixel mode", procName, NULL);
			}

			if (color >= 0) {
				srcAlpha = (int)(srcAlpha * ( (double)GET_DATA_BYTE(&color, L_ALPHA_CHANNEL) / 255));
			}
			...
			if (srcAlpha != 0) {
				...
				new = GET_DATA_BYTE(&color, COLOR_RED);
				old = GET_DATA_BYTE(&pixel, COLOR_RED);
				tmp = (l_uint8)( (double)new * ((double)srcAlpha/255) + (double)old * (1.0 - (double)(srcAlpha)/255) );
				SET_DATA_BYTE(ppixel, COLOR_RED, tmp);

				new = GET_DATA_BYTE(&color, COLOR_GREEN);
				old = GET_DATA_BYTE(&pixel, COLOR_GREEN);
				tmp = (l_uint8)( (double)new * ((double)srcAlpha/255) + (double)old * (1.0 - (double)(srcAlpha)/255) );
				SET_DATA_BYTE(ppixel, COLOR_GREEN, tmp);

				new = GET_DATA_BYTE(&color, COLOR_BLUE);
				old = GET_DATA_BYTE(&pixel, COLOR_BLUE);
				tmp = (l_uint8)( (double)new * ((double)srcAlpha/255) + (double)old * (1.0 - (double)(srcAlpha)/255) );
				SET_DATA_BYTE(ppixel, COLOR_BLUE, tmp);
